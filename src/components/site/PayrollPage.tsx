...
import { PayrollDemo } from "@/components/site/PayrollDemo";
        ...
        <div>
          ...
          <Reveal delay={280}>
            <div className="mt-9 flex flex-wrap gap-3">
              ...
              >
                Talk to a payroll consultant
              </a>
            </div>
          </Reveal>
          <Reveal delay={340}>
            <dl className="mt-12 grid grid-cols-2 gap-6 sm:grid-cols-4">
              {PAY_HERO.stats.map((stat) => (
                <div key={stat.label}>
...
function Modules() {
  ...
  const m = PAY_MODULES[active] ?? PAY_MODULES[0]!;
  return (
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end">
          <Reveal>
            ...
            <h2 className="mt-5 max-w-2xl font-display text-3xl font-extrabold leading-[1.12] text-foreground lg:text-[2.3rem]">
              Fourteen modules. Switch on what your month-end needs.
            </h2>
          </Reveal>
          <Reveal delay={120}>
            <p className="max-w-xs text-sm leading-relaxed text-muted-foreground">
              Browse it the way your HR and finance team would — pick a module to see what it does.
            </p>
          </Reveal>
        </div>
...
function Roles() {
  ...
  const r = PAY_ROLES[active] ?? PAY_ROLES[0]!;
  return (
          ...
          >
            <div className="flex items-center gap-2 border-b border-line bg-mist px-6 py-3.5">
              ...
              <span className="ml-3 text-[11px] font-bold text-muted-foreground">
                {r.label.toLowerCase()} · payroll.yourcompany.in
              </span>
            </div>
...
function Deployment() {
  ...
  const d = cloud ? PAY_DEPLOY.cloud : PAY_DEPLOY.onprem;
  return (
    <section className="bg-mist py-24 lg:py-32">
    ...
    <section className="py-24 lg:py-32">
      <div className="container-x">
        <Reveal>
          ...
          <h2 className="mt-5 max-w-2xl font-display text-3xl font-extrabold leading-[1.12] text-foreground lg:text-[2.3rem]">
            Two kinds of payroll demo. Only one survives month one.
          </h2>
        </Reveal>
          ...
          <Reveal>
            <div className="h-full rounded-[28px] border border-line bg-card p-8 shadow-soft">
              <h3 className="font-display text-xl font-extrabold text-foreground">
                {PAY_COMPARE.typical.title}
              </h3>
              <ul className="mt-6 space-y-4">
                {PAY_COMPARE.typical.points.map((p) => (
          ...
          <Reveal delay={120}>
            <div className="relative h-full overflow-hidden rounded-[28px] bg-navy p-8 text-navy-foreground shadow-lift">
              ...
              <h3 className="relative font-display text-xl font-extrabold text-ember">
                {PAY_COMPARE.acsius.title}
              </h3>
              <ul className="relative mt-6 space-y-4">
                {PAY_COMPARE.acsius.points.map((p) => (
                  <li key={p} className="flex items-start gap-3 text-sm text-navy-foreground/85">
                    <span
                      aria-hidden
                      className="mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded-full bg-ember text-[11px] font-black text-primary-foreground"
    ...
    <section id="why" className="bg-mist py-24 lg:py-32">
      <div className="container-x">
        <Reveal>
          ...
          <h2 className="mt-5 max-w-2xl font-display text-3xl font-extrabold leading-[1.12] text-foreground lg:text-[2.3rem]">
            A payroll partner your finance team can actually reach.
          </h2>
        </Reveal>
        ...
        <div className="grid gap-10 lg:grid-cols-[0.85fr_1.15fr] lg:items-start">
          <div className="lg:sticky lg:top-28">
            <Reveal>
              ...
              <h2 className="mt-5 font-display text-3xl font-extrabold leading-[1.12] text-foreground lg:text-[2.3rem]">
                Payroll questions, answered straight.
              </h2>
              <p className="mt-5 max-w-sm leading-relaxed text-muted-foreground">
                Still comparing vendors? Send us last month's salary sheet — we will run it in a
                sandbox and show you the same numbers, with statutory files attached.
              </p>
