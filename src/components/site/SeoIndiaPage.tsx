        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end">
          <Reveal>
            ...
            <h2 className="mt-5 max-w-3xl font-display text-4xl font-extrabold leading-[1.08] text-foreground lg:text-[3.4rem]">
              What the best SEO services in India actually look like.
            </h2>
          </Reveal>
          <Reveal delay={120}>
            <p className="max-w-sm text-base leading-relaxed text-muted-foreground">
              No lock-ins, no guesswork, no borrowed templates. Four commitments that shape every
              organic SEO programme we run.
            </p>
          </Reveal>
        </div>

        <ul className="mt-14 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {INDIA_WHY.map((item, index) => (
...
function ServiceExplorer() {
  const [active, setActive] = useState(INDIA_SERVICES[0]!.key);
  const current = INDIA_SERVICES.find((s) => s.key === active) ?? INDIA_SERVICES[0]!;
      ...
      <div className="container-x">
        <div className="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end">
          <Reveal>
            ...
            <h2 className="mt-5 max-w-3xl font-display text-4xl font-extrabold leading-[1.08] text-foreground lg:text-[3.4rem]">
              Eight disciplines. One organic growth programme.
            </h2>
          </Reveal>
          <Reveal delay={120}>
            <p className="max-w-sm text-base leading-relaxed text-muted-foreground">
              Hover or tap any discipline to see exactly what is included and the outcome it is
              meant to produce.
            </p>
          </Reveal>
        </div>
        ...
        <div className="mt-14 grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,30rem)]">
          <ul className="grid gap-2 sm:grid-cols-2">
            {INDIA_SERVICES.map((service, index) => {
    ...
    <section id="results" className="py-24 lg:py-32">
      <div className="container-x">
        <Reveal>
          ...
          <h2 className="mt-5 max-w-3xl font-display text-4xl font-extrabold leading-[1.08] text-foreground lg:text-[3.4rem]">
            We report search outcomes, not activity logs.
          </h2>
        </Reveal>

        <ul className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {INDIA_OUTCOMES.map((item, index) => (
...
function Process() {
  ...
  const stage = INDIA_PROCESS[active] ?? INDIA_PROCESS[0]!;
      ...
      <div className="container-x relative">
        <Reveal>
          ...
          <h2 className="mt-5 max-w-3xl font-display text-4xl font-extrabold leading-[1.08] lg:text-[3.4rem]">
            A five-stage SEO programme you can actually track.
          </h2>
        </Reveal>
        ...
        <div className="mt-14 overflow-x-auto pb-4 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          <div className="relative min-w-[760px]">
            ...
            <div
              ...
              style={{ width: `${((active + 1) / INDIA_PROCESS.length) * 100}%` }}
            />
            <ol className="relative grid grid-cols-5 gap-3">
              {INDIA_PROCESS.map((item, index) => {
          ...
          <div className="mt-10 grid gap-6 rounded-[28px] border border-white/12 bg-white/[0.06] p-8 backdrop-blur lg:grid-cols-[auto_1fr_1fr] lg:items-start lg:gap-10">
            ...
            <div className="rounded-2xl border border-navy-foreground/12 bg-navy-foreground/5 p-5">
              <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-ember">
                Stage {active + 1} of {INDIA_PROCESS.length}
              </p>
              <p className="mt-2 text-sm leading-relaxed text-navy-foreground/80">
                Nothing moves to the next stage until the previous one is signed off, so you always
                know what is being worked on and why.
              </p>
    ...
    <section className="bg-mist/70 py-24 lg:py-32">
      <div className="container-x">
        <Reveal>
          ...
          <h2 className="mt-5 max-w-3xl font-display text-4xl font-extrabold leading-[1.08] text-foreground lg:text-[3.4rem]">
            SEO strategies shaped to your sector.
          </h2>
        </Reveal>
        <ul className="mt-14 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {INDIA_INDUSTRIES.map((industry, index) => (
...
function Deliverables() {
  const [active, setActive] = useState(0);
  const item = INDIA_DELIVERABLES[active] ?? INDIA_DELIVERABLES[0]!;
    ...
    <section className="relative overflow-hidden bg-navy py-24 text-navy-foreground lg:py-32">
      ...
      <div className="container-x relative">
        <div className="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end">
          <Reveal>
            ...
            <h2 className="mt-5 max-w-2xl font-display text-3xl font-extrabold leading-[1.08] lg:text-[3rem]">
              Every organic SEO retainer includes these deliverables
            </h2>
          </Reveal>
          <Reveal delay={120}>
            <p className="max-w-sm leading-relaxed text-navy-foreground/70">
              Scope and volume scale with your goals and competition, but nothing on this list is
              optional. Hover any deliverable to see what it means in practice.
            </p>
          </Reveal>
        </div>
        ...
        <div className="mt-14 grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,26rem)]">
          <ul className="grid gap-2 sm:grid-cols-2">
            {INDIA_DELIVERABLES.map((deliverable, index) => {
              const isActive = index === active;
              return (
          ...
          <Reveal delay={140}>
            <div className="h-full rounded-[26px] border border-white/12 bg-white/[0.06] p-8 backdrop-blur">
              ...
              <a href="mailto:info@acsius.com?subject=Request%20from%20acsius.com" className="btn-cta mt-8 inline-block px-6 py-3 text-sm">
                Get a Free SEO Audit
              </a>
            </div>
          </Reveal>
          ...
          <div className="lg:sticky lg:top-28">
            <Reveal>
              ...
              <h2 className="mt-5 max-w-md font-display text-4xl font-extrabold leading-[1.08] text-foreground lg:text-[3.4rem]">
                SEO services in India — your questions answered.
              </h2>
              <p className="mt-5 max-w-sm text-base leading-relaxed text-muted-foreground">
                Straight answers about pricing, timelines, deliverables and how we work as your organic SEO partner.
              </p>
            </Reveal>
          </div>

          <div className="grid gap-4">
            {INDIA_FAQS.map((faq, index) => {
