      ...
      >
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-[1.05fr_0.95fr]">
          <div className="rounded-[20px] border border-white/10 bg-white/5 p-2.5">
            ...
            <div className="overflow-hidden" style={{ height: 172 }}>
              <div className="h-full [&>*]:h-full">
                <SalesGraphDemo />
              </div>
            </div>
          ...
          <div className="rounded-[20px] border border-white/10 bg-white/5 p-4">
            ...
            <ul className="mt-3 space-y-3">
              {[
                { role: "Senior React engineer", exp: "8+ yrs" },
                { role: "Next.js / SSR dev", exp: "6+ yrs" },
                { role: "React + Node full-stack", exp: "5+ yrs" },
                { role: "QA & accessibility", exp: "4+ yrs" },
              ].map((row) => (
                <li key={row.role} className="flex items-center gap-2.5">
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="block text-[10px] text-navy-foreground/60">{row.exp}</span>
                  </span>
                </li>
              ))}
            </ul>
...
/* ---------- Intro: pill tabs + answer cards ---------- */
function Intro() {
  const [active, setActive] = useState(RCT_PILLARS[0]!.key);
  const current = RCT_PILLARS.find((p) => p.key === active) ?? RCT_PILLARS[0]!;
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end">
          <Reveal>
            ...
            <h2 className="mt-5 max-w-3xl font-display text-3xl font-extrabold leading-[1.1] text-foreground lg:text-[2.4rem]">
              A React bench you can hand the front-end to.
            </h2>
          </Reveal>
          <Reveal delay={120}>
            <p className="max-w-xs text-sm leading-relaxed text-muted-foreground">
              Four things teams check before they hire dedicated React JS developers. Tap through.
            </p>
          </Reveal>
        </div>
...
/* ---------- Stack explorer: sidebar list + detail panel ---------- */
function Stack() {
  const [active, setActive] = useState(0);
  const s = RCT_STACK[active] ?? RCT_STACK[0]!;
  return (
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end">
          <Reveal>
            ...
            <h2 className="mt-5 max-w-2xl font-display text-3xl font-extrabold leading-[1.12] text-foreground lg:text-[2.3rem]">
              Eight layers of the front-end. Pick what your product needs.
            </h2>
          </Reveal>
          <Reveal delay={120}>
            <p className="max-w-xs text-sm leading-relaxed text-muted-foreground">
              Browse the stack the way a tech lead would — select a layer to see what ships.
            </p>
          </Reveal>
        </div>
...
/* ---------- Roles: segmented control + app window ---------- */
function Roles() {
  const [active, setActive] = useState(0);
  const r = RCT_ROLES[active] ?? RCT_ROLES[0]!;
  return (
    <section className="py-20 lg:py-28">
      <div className="container-x">
        <Reveal>
          ...
          >
            <div className="flex items-center gap-2 border-b border-line bg-mist px-6 py-3.5">
              ...
              <span className="ml-3 text-[11px] font-bold text-muted-foreground">
                {r.key} · github.com/your-team/app · main
              </span>
            </div>
    ...
    <section className="relative overflow-hidden bg-mist py-20 lg:py-28">
      <div className="container-x">
        <Reveal>
          ...
          <h2 className="mt-5 max-w-2xl font-display text-3xl font-extrabold leading-[1.12] text-foreground lg:text-[2.3rem]">
            You always know what shipped this week.
          </h2>
        </Reveal>
...
/* ---------- Engagement models ---------- */
function Models() {
  const [active, setActive] = useState(RCT_MODELS[0]!.key);
      ...
      <div className="container-x">
        <div className="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end">
          <Reveal>
            ...
            <h2 className="mt-5 max-w-3xl font-display text-3xl font-extrabold leading-[1.1] text-foreground lg:text-[2.4rem]">
              Three ways to hire ReactJS developers.
            </h2>
          </Reveal>
          <Reveal delay={120}>
            <p className="max-w-sm text-base leading-relaxed text-muted-foreground">
              Switch models mid-engagement if the work changes — the same developers stay on your
              account.
            </p>
          </Reveal>
        </div>
        ...
        <Reveal delay={160}>
          <div className="mt-8 flex flex-wrap items-center gap-4">
            ...
            <p className="text-sm text-muted-foreground">
              Building on WordPress or a headless CMS too?{" "}
      ...
      <div className="container-x relative">
        <Reveal>
          ...
          <h2 className="mt-5 max-w-3xl font-display text-3xl font-extrabold leading-[1.1] lg:text-[2.4rem]">
            From first call to first pull request in under a week.
          </h2>
        </Reveal>
        ...
        <Reveal delay={120}>
          <div className="mt-12 flex flex-wrap items-center gap-4">
            <a href={WA} target="_blank" rel="noopener noreferrer" className="btn-cta px-7 py-3.5">
              Message a React Lead on WhatsApp
            </a>
            <p className="text-sm text-navy-foreground/65">
              No lock-in — cancel a dedicated React developer with 15 days' notice.
            </p>
          </div>
        </Reveal>
      </div>
    ...
    <section className="py-20 lg:py-28">
      <div className="container-x">
        <Reveal>
          ...
          <h2 className="mt-5 max-w-2xl font-display text-3xl font-extrabold leading-[1.12] text-foreground lg:text-[2.3rem]">
            Two React quotes. Only one is still maintainable in month six.
          </h2>
        </Reveal>
          ...
          <Reveal>
            <div className="h-full rounded-[28px] border border-line bg-card p-8 shadow-soft">
              <h3 className="font-display text-xl font-extrabold text-foreground">
                {RCT_COMPARE.typical.title}
              </h3>
              <ul className="mt-6 space-y-4">
                {RCT_COMPARE.typical.points.map((p) => (
          ...
          <Reveal delay={120}>
            <div className="relative h-full overflow-hidden rounded-[28px] bg-navy p-8 text-navy-foreground shadow-lift">
              ...
              <h3 className="relative font-display text-xl font-extrabold">
                {RCT_COMPARE.acsius.title}
              </h3>
              <ul className="relative mt-6 space-y-4">
                {RCT_COMPARE.acsius.points.map((p) => (
    ...
    <section className="bg-mist py-20 lg:py-28">
      <div className="container-x">
        <Reveal>
          ...
          <h2 className="mt-5 max-w-3xl font-display text-3xl font-extrabold leading-[1.1] text-foreground lg:text-[2.4rem]">
            The commitments behind every React engagement.
          </h2>
        </Reveal>

        <ul className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {RCT_OUTCOMES.map((item, i) => (
    ...
    <section id="why" className="py-20 lg:py-28">
      <div className="container-x">
        <Reveal>
          ...
          <h2 className="mt-5 max-w-3xl font-display text-3xl font-extrabold leading-[1.1] text-foreground lg:text-[2.4rem]">
            Hire dedicated React JS developers without the usual risk.
          </h2>
        </Reveal>
        ...
        <Reveal delay={140}>
          <p className="mt-8 max-w-3xl text-sm leading-relaxed text-muted-foreground">
            Need the API side too? Pair your React front-end with our{" "}
            <Link to="/hire-laravel-developers" className="font-bold text-primary hover:text-ember">
              Laravel developers
            </Link>{" "}
            or{" "}
            <Link to="/hire-php-developers" className="font-bold text-primary hover:text-ember">
              PHP developers
            </Link>
            . For simple marketing sites your team edits daily, our{" "}
            <Link to="/hire-wix-developers" className="font-bold text-primary hover:text-ember">
              Wix developers
            </Link>{" "}
            are usually the cheaper answer.
          </p>
        </Reveal>
      </div>
    </section>
    ...
    <section className="bg-mist py-20 lg:py-28">
      <div className="container-x">
        <Reveal>
          ...
          <h2 className="mt-5 max-w-3xl font-display text-3xl font-extrabold leading-[1.1] text-foreground lg:text-[2.4rem]">
            Sector context, not a generic starter kit.
          </h2>
        </Reveal>

        <div className="mt-10 grid gap-x-10 md:grid-cols-2">
          {RCT_INDUSTRIES.map((item, i) => (
...
/** Interlinking band to related ACSIUS hiring pages. */
function NextSteps() {
  return (
    <section className="relative overflow-hidden bg-navy py-20 text-navy-foreground lg:py-28">
      ...
      <div className="container-x relative">
        <Reveal>
          ...
          <h2 className="mt-5 max-w-3xl font-display text-3xl font-extrabold leading-[1.1] lg:text-[2.4rem]">
            One studio for the whole stack.
          </h2>
        </Reveal>
          ...
          <div className="lg:sticky lg:top-28">
            <Reveal>
              ...
              <h2 className="mt-5 max-w-md font-display text-3xl font-extrabold leading-[1.1] text-foreground lg:text-[2.4rem]">
                Hiring ReactJS developers, answered.
              </h2>
              <p className="mt-5 max-w-sm text-base leading-relaxed text-muted-foreground">
                Still unsure which model fits? Send your repo or backlog and we'll tell you what we
                would staff and why.
              </p>
              <a
                href={WA}
              ...
              >
                Ask a React Lead on WhatsApp
              </a>
            </Reveal>
          </div>

          <div className="grid gap-4">
            {RCT_FAQS.map((faq, index) => {
