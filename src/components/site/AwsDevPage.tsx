...
import { AwsDemo } from "@/components/site/AwsDemo";
...
/* Split-list explorer: vertical rail of tracks + dark detail panel */
function Capabilities() {
  const [active, setActive] = useState(0);
  const cap = AWS_CAPABILITIES[active] ?? AWS_CAPABILITIES[0]!;
  return (
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end">
          <Reveal>
            ...
            <h2 className="mt-5 max-w-3xl font-display text-3xl font-extrabold leading-[1.12] text-foreground lg:text-[2.3rem]">
              Five tracks we staff — pick where you need hands.
            </h2>
          </Reveal>
          <Reveal delay={120}>
            <p className="max-w-xs text-sm leading-relaxed text-muted-foreground">
              Every engagement is scoped in writing, with an AWS run-rate estimate attached.
            </p>
          </Reveal>
        </div>

        <div className="mt-12 grid gap-4 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
          <ul className="overflow-hidden rounded-[26px] border border-line bg-card shadow-soft">
            {AWS_CAPABILITIES.map((c, i) => {
              const on = active === i;
              return (
                <li key={c.key} className={i > 0 ? "border-t border-line" : ""}>
                  <button
                    type="button"
                    onClick={() => setActive(i)}
                    aria-pressed={on}
                    className={`flex w-full cursor-pointer items-center gap-4 px-6 py-5 text-left transition-colors duration-400 ${
                      on ? "bg-navy text-navy-foreground" : "hover:bg-mist/70"
                    }`}
                  >
                    <span className={`font-display text-xs font-black ${on ? "text-ember" : "text-ember/80"}`}>
                      {String(i + 1).padStart(2, "0")}
                    </span>
                    <span
                      className={`flex-1 font-display text-base font-extrabold ${on ? "text-navy-foreground" : "text-foreground"}`}
                    >
                      {c.tab}
                    </span>
                    <span
                      aria-hidden
                      className={`shrink-0 text-sm font-black transition-transform duration-300 ${on ? "translate-x-1 text-ember" : "text-primary"}`}
                    >
                      →
                    </span>
                  </button>
          ...
          <div
            key={cap.key}
            className="animate-[fadeUp_0.5s_ease-out_both] overflow-hidden rounded-[26px] border border-transparent bg-navy text-navy-foreground shadow-lift"
          >
            ...
            <div className="flex flex-wrap items-end justify-between gap-4 border-t border-white/10 bg-white/[0.04] px-8 py-6 lg:px-10">
              <div>
                <p className="font-display text-3xl font-extrabold text-ember lg:text-4xl">
                  {cap.metric}
                </p>
              ...
              <a href="mailto:info@acsius.com?subject=Request%20from%20acsius.com" className="text-sm font-bold text-ember">
                Scope this track →
              </a>
            </div>
          </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end">
          <Reveal>
            ...
            <h2 className="mt-5 max-w-2xl font-display text-3xl font-extrabold leading-[1.12] text-foreground lg:text-[2.3rem]">
              Roles you can add to your cloud team.
            </h2>
          </Reveal>
          <Reveal delay={120}>
    ...
    <section className="py-20 lg:py-24">
      <div className="container-x grid gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:items-center">
        <Reveal>
          <p className="eyebrow">
            <span className="h-px w-8 bg-ember" />
            The services we run daily
          </p>
          <h2 className="mt-5 max-w-sm font-display text-2xl font-extrabold leading-[1.15] text-foreground lg:text-[2rem]">
            Managed in Terraform, monitored in CloudWatch.
          </h2>
          <p className="mt-5 max-w-sm text-sm leading-relaxed text-muted-foreground">
            No CV padding. If a service is on this list, someone on the bench ran it in production
            this year.
          </p>
        </Reveal>
        <Reveal delay={100}>
          <ul className="flex flex-wrap gap-2.5">
            {AWS_STACK.map((tool, i) => (
              <li
                key={tool}
    ...
    <section className="relative overflow-hidden bg-navy py-20 text-navy-foreground lg:py-24">
      ...
      <div className="container-x relative">
        <Reveal>
          <p className="eyebrow text-ember">
            <span className="h-px w-8 bg-ember" />
            AWS development company · engagement models
          </p>
        ...
        <Reveal delay={200}>
          <a href="https://wa.me/919891764802" target="_blank" rel="noopener noreferrer" className="btn-cta mt-10 inline-flex px-7 py-3.5">
            Hire an AWS certified developer
          </a>
        </Reveal>
      </div>
    </section>
        ...
        <Reveal>
          <div className="lg:sticky lg:top-28">
            ...
            <h2 className="mt-5 max-w-sm font-display text-3xl font-extrabold leading-[1.12] text-foreground lg:text-[2.3rem]">
              An architecture you approve before we build it.
            </h2>
      ...
      <div className="container-x grid gap-12 lg:grid-cols-[0.85fr_1.15fr] lg:items-center">
        <Reveal>
          ...
          <h2 className="mt-5 max-w-md font-display text-3xl font-extrabold leading-[1.12] text-foreground lg:text-[2.3rem]">
            Hire AWS developers in India, not a marketplace listing.
          </h2>
          <a href="https://wa.me/919891764802" target="_blank" rel="noopener noreferrer" className="btn-cta mt-8 inline-flex px-7 py-3.5">
          ...
          <div className="lg:sticky lg:top-28">
            <Reveal>
              ...
              <p className="mt-5 max-w-sm text-base leading-relaxed text-muted-foreground">
                Share your workload or brief. You get matched profiles, an engagement plan and a
